const SelectField = ({
  ...
  const baseStyles = "w-full py-2 px-4 pr-10 text-black transition-all duration-200 focus:outline-none appearance-none text-white";

  // Variant-specific styles
  const variants = {
    default: "border border-card-border bg-card rounded-lg focus:border-primary-light focus:outline-0 focus:ring-2 focus:ring-primary/50 focus:border-blue-500 focus:border-transparent",
    outline: "border-b-2 border-gray-300 focus:border-blue-500 bg-card border-gray-600  focus:border-blue-500",
  ...
  const renderOptions = () => {
    return options.map((option, index) => {
      ...
      return (
        <option key={index} value={option.name || JSON.stringify(option)} className="">
    ...
    return (
      <div className="relative">
        <select
          ...
          required={isRequired}
        >
          <option value="" className="dark:bg-gray-700"></option>
          {renderOptions()}
        </select>
        <label className={`absolute left-4 text-slate-500 pointer-events-none transition-all duration-200 
  ...
  return (
    <div className="relative">
      ...
      <div className="relative">
        ...
        <ChevronIcon />
        {icon && <div className="absolute left-0 top-3.5 text-slate-500">{icon}</div>}
      </div>
      {error && <p className="mt-1 text-sm text-red-500 dark:text-red-400">{error}</p>}
    </div>
  );
